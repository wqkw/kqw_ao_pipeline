...
def create_shots_collage(artifact_or_json: Union[ShortStoryboardSpec, dict, str], output_filename: Optional[str] = None) -> str:
    ...
    # Collect all shots with images from the single scene
    shots_with_images = []
        ...
        for shot in artifact.scene.shots:
            if shot.image_path:
                ...
                if image_path.exists():
                    shots_with_images.append((artifact.scene.name, shot, image_path))
...
def artifact_to_shot_list_string(artifact: ShortStoryboardSpec) -> str:
    ...
    if not artifact.scene:
        return "No scene found in artifact."

    # Add motion and narrative info
    if artifact.motion_level or artifact.narrative_level:
        output_lines.append(f"Motion Level: {artifact.motion_level or 'N/A'}")
        output_lines.append(f"Narrative Level: {artifact.narrative_level or 'N/A'}")
        output_lines.append("")

    output_lines.append(f"SCENE: {artifact.scene.name}")
    output_lines.append("=" * 60)
    output_lines.append("")

    if not artifact.scene.shots:
        output_lines.append("No shots in this scene.")
        return "\n".join(output_lines)

    for shot_idx, shot in enumerate(artifact.scene.shots, 1):
