...
def extract_context_dto(artifact: ShortStoryboardSpec, step, **kwargs) -> BaseModel:
    ...
    if step == GenerationStep.IMAGE_RECOGNITION:
        ...
        ContextModel = create_model("MotionNarrativeAnalysisContext", **fields, __base__=StrictModel)
        data = kwargs.copy()
        ...
        fields = {
            ...
            "narrative_level": (str, Field(..., description="Narrative level: low/medium/high")),
            "image_descriptions": (List[str], Field(..., description="AI-generated descriptions of input images"))
        }
        ContextModel = create_model("ShotsContext", **fields, __base__=StrictModel)
        data = kwargs.copy()
        data["motion_level"] = artifact.motion_level or "medium"
        data["narrative_level"] = artifact.narrative_level or "medium"
        data["image_descriptions"] = artifact.image_descriptions or []
        return ContextModel(**data)
        ...
        fields = {
            "prompt_input": (str, Field(..., description="User's creative input for this generation step")),
            "scene": (SceneSpec, Field(..., description="Scene with shots to generate images for")),
...
def create_output_dto(step) -> Type[StrictModel] | None:
    ...
    if step == GenerationStep.IMAGE_RECOGNITION:
        ...
        fields["motion_level"] = (str, Field(..., description="Motion level: low/medium/high"))
        fields["narrative_level"] = (str, Field(..., description="Narrative level: low/medium/high"))
        fields["analysis"] = (str, Field(..., description="Brief explanation of the motion and narrative level choices"))
...
def patch_artifact(artifact: ShortStoryboardSpec, step, output: BaseModel) -> ShortStoryboardSpec:
    ...
    if step == GenerationStep.IMAGE_RECOGNITION:
        ...
        artifact.motion_level = output.motion_level
        artifact.narrative_level = output.narrative_level

    # SHOTS, SHOT_IMAGES, and FINAL_IMAGE are handled directly in pipeline
    # (shot generation, image generation, collage creation)
...
def get_dependencies():
    ...
    return {
        ...
        GenerationStep.MOTION_NARRATIVE_ANALYSIS: [GenerationStep.IMAGE_RECOGNITION],
        GenerationStep.SHOTS: [GenerationStep.MOTION_NARRATIVE_ANALYSIS],
...
def get_completion_status(artifact: ShortStoryboardSpec) -> Dict:
    ...
    # Check if scene has shots
    scene_has_shots = False
    if artifact.scene and artifact.scene.shots and len(artifact.scene.shots) > 0:
        scene_has_shots = True

    # Check if all shots have images
    all_shots_have_images = False
    if scene_has_shots and artifact.scene.shots:
        all_shots_have_images = all(
            shot.image_path is not None
            for shot in artifact.scene.shots
        )

    return {
        GenerationStep.IMAGE_RECOGNITION: artifact.image_descriptions is not None and len(artifact.image_descriptions) > 0,
        GenerationStep.MOTION_NARRATIVE_ANALYSIS: artifact.motion_level is not None and artifact.narrative_level is not None,
        GenerationStep.SHOTS: scene_has_shots,
