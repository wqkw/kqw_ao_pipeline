...
from storyboard_shotonly import (
    ShortStoryboardSpec,
    GenerationStep,
    generate_step, generate_shots, generate_shot_images,
    patch_artifact
)
from storyboard_shotonly.utils import create_shots_collage, artifact_to_shot_list_string

load_dotenv()


async def process_single_image(image_path: str, output_base_dir: Path) -> tuple[str | None, ShortStoryboardSpec | None]:
    """Process a single image through the full shotonly pipeline including image generation and collage.
    ...
    run_name = f"shotonly_storyboard_{image_name}"
...
async def main():
    ...
    input_dir = Path("data/ref_moodboard5")
    output_base_dir = Path("data")
    storyboards_collection_dir = Path("data/storyboards_shotonly_collection")
    ...
    print(f"📁 Individual outputs in: {output_base_dir}/shotonly_storyboard_*/")
